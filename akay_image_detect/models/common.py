...
from utils.plots import color_list, plot_one_box, plot_one_box_PIL
...
class Detections:
    ...
    def __init__(self, imgs, pred, files, times=None, names=None, shape=None):
        ...
        with open("./json_files/coco_classes.json", encoding="utf-8") as f:
            ...
            for json_object in json_content["classes"]:
                self.classes_dic.append([json_object["id"], json_object["turkish"]])
    ...
    def display(self, pprint=False, show=False, save=False, render=False, save_dir='', get_img=False):
        colors = color_list()
        for i, (img, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            if pred is not None:
                ...
                if show or save or render or get_img:
                    for *box, conf, cls in pred:  # xyxy, confidence, class
                        label = f'{self.names[int(cls)]} {conf:.2f}'
                        for j in self.classes_dic:
                            if label.split(" ")[0] == j[0]:
                                label = label.replace(j[0], j[1])
                        img = plot_one_box_PIL(box, img, label=label, color=colors[int(cls) % 10])
    ...
    def get_image(self):
        return self.display(get_img=True)  
